...
<html lang="en">
  ...
  <tbody>
  <% 
  int department_id = (Integer) request.getAttribute("department_id");
  int semester_id = (Integer) request.getAttribute("semester_id");
  int division_id = (Integer) request.getAttribute("division_id");
  %>
  <%List<Studentcomplete> list = AdminDao.getAllCompleteddatabydsd(department_id,semester_id,division_id); %>
  <%for(Studentcomplete u1:list){ %>
    <tr>
    	<td><%=u1.getId() %></td>
    	<td><%=u1.getStu_id() %></td>
    	<td><%=u1.getCo_id() %></td>
    	...
    	<td>
    		<form action="AdminController" method="post">
    			...
    			<input type="hidden" name="action" value="completed_deletee">
